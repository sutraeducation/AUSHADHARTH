...
function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className={`app-layout ${collapsed ? "app-layout--collapsed" : ""}`}><aside className="sidebar" aria-label="Primary navigation"><div className="sidebar__brand"><BrandMark compact={collapsed} /></div><nav><NavLink to="/app/dashboard" className={({ isActive }) => `nav-item ${isActive ? "nav-item--active" : ""}`}><DashboardIcon /><span>Dashboard</span></NavLink><span className="nav-group-label">{collapsed ? "" : "MASTERS"}</span><NavLink to="/app/reference" className={({ isActive }) => `nav-item ${isActive ? "nav-item--active" : ""}`}><ReferenceIcon /><span>Reference Data</span></NavLink></nav>{!collapsed && <div className="sidebar__next"><span>NEXT MODULES</span><p>Product, billing, inventory, and accounts remain unavailable until their implementation slices are complete.</p></div>}<button className="sidebar-toggle" type="button" onClick={toggleSidebar} aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"} aria-expanded={!collapsed}><span aria-hidden="true">{collapsed ? "›" : "‹"}</span><span>{collapsed ? "" : "Collapse"}</span></button></aside><div className="workspace"><header className="topbar"><div><span className="topbar__label">WORKSPACE</span><strong>{auth.status?.storeDisplayName ?? "AUSHADHARTH"}</strong></div><div className="topbar__actions"><ConnectivityStatus /><div className="user-menu"><button ref={menuButton} className="user-button" type="button" onClick={() => setMenuOpen(!menuOpen)} aria-haspopup="menu" aria-expanded={menuOpen}><span className="avatar" aria-hidden="true">{initials(user?.displayName)}</span><span><strong>{user?.displayName}</strong><small>{roleLabel(user?.role)}</small></span><span aria-hidden="true">⌄</span></button>{menuOpen && <div className="user-popover" role="menu"><p><strong>{user?.displayName}</strong><small>{user?.loginIdentifier}</small></p><button role="menuitem" type="button" onClick={() => void auth.logout()}>Sign out</button></div>}</div></div></header><main className="main-content">{children}</main></div></div>;
}
